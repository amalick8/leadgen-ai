...
export async function signUpBusiness(_prev: AuthState, formData: FormData): Promise<AuthState> {
  ...
  try {
    ...
    return { success: false, error: "Supabase auth is not configured yet. Add your Supabase URL, anon key, and service role key to enable signups." };
  }
...
export async function login(_prev: AuthState, formData: FormData): Promise<AuthState> {
  ...
  if (!parsed.success) return { success: false, fieldErrors: parsed.error.flatten().fieldErrors };
  let destination = "/business/onboarding";
  try {
    const supabase = await createSupabaseServerClient();
    ...
    if (profile.role !== "admin") destination = business?.onboarding_completed ? "/business/dashboard" : "/business/onboarding";
  } catch {
    return { success: false, error: "Supabase auth is not configured yet. Add your environment variables before logging in." };
  }
